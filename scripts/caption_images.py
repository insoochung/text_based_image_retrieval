...
def maybe_download(s3_client, photo, images_dir):
    image_name = photo.image_url.split("/")[-1]
    image_path = f"{images_dir}/{image_name}"
    if not os.path.exists(image_path):
        s3_client.download_file(
            AWS_STORAGE_BUCKET_NAME, image_name, image_path)
    return image_path
...
def caption_images(images_dir="images"):
    ...
    for photo in tqdm.tqdm(Photo.objects.all()):
        if len(photo.caption):
            continue
        image_path = maybe_download(s3_client, photo, images_dir)
        try:
